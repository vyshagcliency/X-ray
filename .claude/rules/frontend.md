...
- **Web report = light SaaS workspace (2026-07-07, Vyshag, supersedes the 3-zone reading shell):** the **web** report is a sticky top app bar (`ReportTopBar.tsx`: identity + recoverable-now + Book-a-call + PDF/CSV) over three tabs (`ReportShell.tsx`: Overview / Findings / Deadlines). Findings is a filterable category table (`FindingsTab.tsx`) whose rows open a drawer (`ReportDrawer.tsx`) holding the full per-category dossier (`CategoryDeepDive.tsx`, de-boxed). No colored container borders anywhere: category color appears only as an 8px dot or a chart bar. This chrome is **web-only**; the **PDF/print deliverable stays a single linear document**, and `data-builder.ts` + both PDF renderers are untouched, so parity holds. See decisions.md 2026-07-07.
...
- No SaaS-style sidebars, tabs, or navigation on the **funnel** pages (landing, start, upload, processing). **Exception (2026-07-07, Vyshag):** the **web report** is a light SaaS workspace, a sticky top app bar + Overview/Findings/Deadlines tabs + a findings drawer (`ReportTopBar`/`ReportShell`/`FindingsTab`/`ReportDrawer`), web-only. The PDF/print deliverable stays a single linear document. See the Exception section above.
