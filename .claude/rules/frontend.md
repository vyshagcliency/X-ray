...
- No SaaS-style sidebars, tabs, or navigation on the **funnel** pages (landing, start, upload, processing). **Exception (2026-07-08, Vyshag):** the **web report** is a light Stripe-style dashboard, a left sidebar + Overview/Findings/Deadlines + a findings drawer (`ReportSidebar`/`ReportShell`/`FindingsTab`/`ReportDrawer`), web-only. The PDF/print deliverable stays a single linear document. See the Exception section above.
