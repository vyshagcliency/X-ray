...
| 2026-07-08 | **Report hero + exec summary lead with the PROVABLE figure, not the soft all-in total; dense text blocks made visual** | Owner: "$230/mo sounds too small to care," and the exec summary / surfaced-total strip read too text-heavy. **Web (presentational, hits existing reports):** the Overview hero now leads with the provable one-time **recoverable-now** figure (was the $230/mo run-rate); the "surfaced in total" sentence became a provable-vs-estimated **split bar**; the exec summary chips became **figure tiles** (Discrepancies / Provable / Time-sensitive). **Narrative (baked, new/re-run audits only):** `narrate.ts` exec summary now leads with `provable_cents` (new field on `NarrativeFigures`/`computeNarrativeFigures`, passed via `audit-run.ts`) instead of `total_recoverable_cents`, is shorter, and its em dashes are scrubbed. Reverses the 2026-07-05 "hero = provable-forward run-rate" lock for the web hero; the run-rate stays as the "still bleeding" secondary hook + on the Deadlines tab. Number parity preserved (provable_cents is the same non-estimated sum data-builder uses). Requires a Trigger.dev worker redeploy to reach new audits; existing reports keep old prose until re-run. |
| 2026-07-08 | **Stripe dashboard refinements (owner feedback round 2)** | Four changes on top of the Stripe dashboard row below. (1) **Accent recolored** `#635bff` → **`#4971ff`** and unified in one place (`ACCENT` + `CTA_CLASS` in `DashboardCard.tsx`) so the Book-a-call CTA, active nav, and hero chart share one blue (was two competing blues). (2) **Trust cards + exec summary + method de-densified**: the confidence card is now a color-dot legend, "traces to a row" shows `Order/SKU/Date` chips, the exec summary gets an at-a-glance figure strip, and "How we found this" gets a 3-step method row + "reports we read" chips — content reorganized/illustrated, none removed. (3) **Findings drilldown is now a full in-place detail page** (list swaps to the `CategoryDeepDive` dossier with an "All findings" back button + Escape) instead of a side drawer; **`ReportDrawer` retired/deleted**. (4) **Deadlines** gains a KPI stat-tile strip (time-sensitive $, on-the-clock count, recurring drain, 12-mo projection) above the charts. Pure presentation; numbers/PDF/data untouched; build+lint+types+180 tests green + live render smoke test. |
| 2026-07-08 | **Web report re-cast again into a light Stripe-style dashboard (left sidebar + full-bleed bento of cards) — supersedes the same-day top-tab SaaS-workspace row below** | Owner feedback on the top-tab v1: dead side gutters (content capped at `max-w-6xl` and centered), wanted a left side menu not top tabs, and it read flat/boring because v1 stripped all containers. Owner reference: **the Stripe dashboard**. **Change:** app shell — `ReportSidebar` (brand + case, Overview/Findings/Deadlines nav, pinned recoverable-now + Book-a-call + PDF/CSV; `ReportMobileBar` under `lg`) + a fluid main capped at `max-w-[1600px]`. Content is a bento of cards from two new primitives, `DashboardCard` + `StatTile`, over a `slate-50` canvas: a Stripe big-metric + area-chart hero (reuses the now-exported `ForwardBleedChart`), KPI tiles, chart cards (`ForensicVisuals` `ChartBlock` is now a card), a featured sharpest-finding card, and a Findings data-table card → `ReportDrawer` dossier. One shared card treatment (`CARD_CLASS`) and one reserved indigo accent (`#635bff`, `ACCENT`) for CTA / active nav / hero chart; still no colored container borders, still no em dashes. Retired `ReportTopBar`. **Preserved:** light palette, tiered story, and full web↔PDF number parity — `data-builder.ts` + both PDF renderers untouched; PDF stays a single linear document; pure presentation, existing reports re-render from stored `report_data`. Spec/plan in `docs/superpowers/` + `~/.claude/plans/`. |
| 2026-07-07 | **Web report re-cast from the 3-zone reading document into a light SaaS workspace (top app bar + Overview/Findings/Deadlines tabs + findings drawer) — supersedes the same-day 3-zone-shell row below** | Owner directive (Vyshag): the report still read as "AI UI" and felt like a long page. **Change:** `page.tsx` keeps every `report_data` derivation but now renders a client `ReportShell` (`ReportTopBar` identity + recoverable-now + Book-a-call + PDF/CSV over Overview/Findings/Deadlines tabs). Overview carries the full pitch (hero, KPIs, sharpest finding, money charts, trust, exec summary, method note, close). Findings is a filterable category table whose rows open a hand-rolled accessible `ReportDrawer` with the full `CategoryDeepDive` dossier. Deadlines carries the urgency charts + a new provable-only "closing within 14 days" list (`deriveClosingSoon`) + CTA. **Killed the AI tells:** removed every colored container border (the per-category `borderTop`/`borderLeft` accent stripes) and the uniform white-card stack; category color now shows only as an 8px dot or a chart bar; retired `ReportNav` + `ReportDock`. **Also locked:** no em dashes in customer-facing copy (scrubbed `category-meta.ts`/`finding-math.ts`/component copy + a `stripEmDashes` render-boundary scrub on baked narrative). No new dependencies (tabs + drawer hand-rolled). **Preserved:** the light palette, the tiered story, and full web↔PDF number parity — `data-builder.ts` + both PDF renderers untouched; the PDF/print deliverable stays a single linear document. Overrides the P4.2 "reads like a document, not a dashboard" lock for the **web** surface only. Spec + plan in `docs/superpowers/`. |
