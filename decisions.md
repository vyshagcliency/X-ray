...
| LLM model | Anthropic Claude (`claude-sonnet-4-6` primary, `claude-haiku-4-5-20251001` bulk) | Latest + cost/latency split. Bumped from Sonnet 4.5 → 4.6 on 2026-04-18 during pre-build tech re-validation. |
...
| 2026-04-18 | **Model bump: Sonnet 4.5 → `claude-sonnet-4-6`** (Haiku 4.5 stays) | Sonnet 4.6 is a drop-in with the same pricing profile and equal-or-better reasoning on our workload (narrative prose around structured data). No prompt changes required. |
