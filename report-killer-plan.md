...
- [x] **P2.1 — Dossier template** in `CategoryDeepDive` (or a new `FindingDossier` component): the 5-part structure (what happened / evidence rows / math shown / how-to-file + window / confidence & why). Extract per-category "how to file" (Amazon case path) into `category-meta.ts`. **DONE 2026-07-06:** `CategoryDeepDive` restructured into the explicit 5-part dossier — header/mechanism (what happened) → evidence table (unchanged) → a new slate-panel footer holding **The math, shown** → **How to file it** → **Confidence & why**. `category-meta.ts` gained `filePath` (Amazon case path), `disputeWindow` (real deadline) and `confidenceWhy` (honest per-category rationale) for all 6 categories + the fallback. The how-to-file copy was **web-researched** (2026-07, not assumed per the master prompt's format-research rule): fee disputes = ~90-day window + multi-unit Cubiscan re-measure for size-tier; reimbursements = up to 18 months via the FBA case log.
- [x] **P2.2 — Show the math.** For each finding, render the computation, not just the result (e.g. referral: "charged 15% × $X revenue = $A; category rate 8% = $B; overcharge $A−$B"). All figures already in `evidence`. **DONE 2026-07-06:** new pure, tested `src/components/report/finding-math.ts` → `financeMath(category, evidence, amountCents)` returns `{ formula, rows[] }` for all 6 categories, worked from the category's largest (deterministic) case; the emphasized result row always reconciles to `amount_cents` (regression-tested). **Single source:** `Spotlight` now renders its math strip from `financeMath` too, so the wedge arithmetic can't diverge between spotlight and dossier. **Credibility fix:** the shown figures use `formatDollarsExact` (2-decimal) so the on-screen subtraction self-reconciles to the penny ($76.38 − $40.74 = $35.64) — whole-dollar rounding made the referral example read $76 − $41 = $36, which a verify-everything CFO would catch. The scannable evidence table stays whole-dollar (worked-example-exact vs ledger-scannable — standard).
- [x] **P2.3 — Surface the dispute draft.** The top-25 `draft-dispute` output is computed and stored but not shown on the web report — expose it per finding (copy-ready), or link to it in the PDF/CSV. **DONE 2026-07-06:** did BOTH paths. Each dossier's "How to file it" surfaces a **copy-ready draft** for the category's lead finding via a collapsed client `DisputeDraftBlock` (+ a reusable `CopyButton` island); the CSV export carries a draft for **every** finding (the long tail). Drafts are generated server-side via the existing `draftDispute` template — no client calculation, amounts trace to `amount_cents`.
- [x] **P2.4 — CSV export** of all findings (the report references "CSV export" but verify it exists; if not, add it — it's part of "yours to file, free"). **DONE 2026-07-06:** it did NOT exist — the report copy ("Full detail is in the PDF and CSV export") referenced a missing endpoint. Added `GET /api/audit/csv?id=` (paginated-complete fetch, same 1000-row PostgREST guard as the page) + a wired "Export CSV" button in the hero. Serialization is a new pure, tested `src/lib/report/findings-csv.ts` (RFC-4180 escaping so multi-line dispute bodies stay one record; amount as a plain summable decimal; UTF-8 BOM for Excel; a copy-ready dispute message per row; **CSV formula-injection neutralized** — seller-uploaded cells beginning `=`/`+`/`-`/`@`/tab/CR are quote-prefixed, since the export is forwarded and opened in Excel/Sheets — flagged by the background security review, fixed TDD).

**Exit gate:** ✅ **MET 2026-07-06.** Each finding is independently understandable and filable from the report alone — a Controller has, per category: the plain-English mechanism, the exact evidence rows, the self-reconciling math, the Amazon case path + real window (+ ≤14d urgency count), a copy-ready dispute draft, and an honest confidence rationale; and the CSV carries all of that per finding for the long tail. `pnpm build` (✓ 0 errors) + `pnpm lint` (clean) + `pnpm test` (113 pass, +12 this phase: 6 `finding-math` + 6 `findings-csv`, zero regressions). Halcyon dossiers eyeballed via a real-pipeline-output Artifact preview (every figure is live output; numbers reconcile). *Live in-browser render still pending deploy (as with P0.5/P1); the dossier structure + reconciled numbers are proven via the preview + the pure-module tests. Awaiting user go-ahead before Phase 3/4.*
