...
| `ua` | `text` | Same. |
| `report_data` | `jsonb` | **The single source of truth for every report number** (web + PDF both render from it; nothing re-aggregates `findings`). Built once by `pdf/data-builder.ts` from the complete in-memory finding set (NOT the DB round-trip, which PostgREST caps at 1000 rows). Holds: narrative; `categories` **ordered by confidence×punch** (`high_cents` desc, then `$` — P1.3; each with count/total/urgent + confidence breakdown + `high_cents` + `recurring`/`estimated` flags); top-level `confidence` tally; `total`/`provable`/`estimated`/`recurring`/`one_time` cents; `provable_one_time_cents`/`provable_urgent_cents`; the **hero** `provable_forward_cents` + `provable_forward_monthly_cents` (high-confidence rolling ÷ months — P1.1); `provable_confidence_cents` + `urgency_buckets` (chart aggregates — P1.6); `spotlight` (the sharpest wedge finding — P1.2); `skus_affected`; `top_cases` (25, each with a dispute draft); and **`pdf`** — a precomputed **print-ready view** (`PdfView`) that is the single source for **both** PDF renderers (Typst + React-PDF): pre-formatted hero/surfaced strings, spotlight (claim + shown-math rows), and provable/estimated category dossiers (mechanism, how-to-file, dispute window, confidence-why, worked math), all built with the same `formatDollars`/`catMeta`/`financeMath` the web uses so web ↔ PDF can't drift (P4.1). A dev/test invariant (`assertReportDataConsistent`) enforces Σcategory==total, confidence==findings_count, provable+estimated==total, Σprovable-confidence==provable, and pdf-category-partition==categories. Added 2026-04-21; reshaped by Report Killer P0.2–P0.4 + P1.1–P1.6 (2026-07-06) + the `pdf` view P4.1 (2026-07-07). |
